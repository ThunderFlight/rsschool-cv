...
export const ModalRegister = ({regWrapPos, regWrap, openRegState, openModalYouHaveAccount, closeModalRegister}) => {
  ...
  const {users, createUser} = useAppContext();  
  return (
  <Portal htmlLink='nav'>
    <div className={openRegState  ? classNames(styles[regWrapPos], styles[regWrap]) : styles.modalRegisterWrapClose}>
          <form className={styles.regandlogform} onSubmit={handleSubmit((data) => {createUser(reset, closeModalRegister, data)})}>
          ...
          <div className={styles.regandlogform__form}>
              {inputPropsList.map((item,id)=>{
                return (
                  <FormInput errors={errors} name={item.name} pattern={item.pattern} register={register} key={id} id={uid()} label={item.label}/>
                )
              })}
            ...
            <p>
              ...
              <button onClick={() => openModalYouHaveAccount()}>
                Login
              </button>
            </p>
          </div>
